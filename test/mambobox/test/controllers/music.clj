...
(deftest test-music-search
  (let [all-songs [{:name "Vente negra"
                   ...
                   {:name "No te puedo querer"
                    :artist "La sucursal"
                    :tags ["rumba"]}]
        result1 (:songs-found (search-music "user" "" "" 1 10 all-songs))
        result2 (:songs-found (search-music "user" "ente negra" "" 1 10 all-songs))
        result3 (:songs-found (search-music "user" "sucurzal" "" 1 10 all-songs))
        result4 (:songs-found (search-music "user" "" "son" 1 10 all-songs))
        result5 (:songs-found (search-music "user" "" "" 1 2 all-songs))]
    (is (= (count result1) 3) "All")
    (is (= (count result2) 1) "With query name")
    (is (= (count result3) 2) "With query artist")
    (is (= (count result4) 2) "With tag")
    (is (= (count result5) 2) "All with page")))
...
;;     (is (= (make-suggesteds-ids-vector my-favs all-favs  most-visited 2) [5 6 7 8]))
;;     (is (= (make-suggesteds-ids-vector my-favs all-favs  most-visited 10) [5 6 7 8 15]))))
